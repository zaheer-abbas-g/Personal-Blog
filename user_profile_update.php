...
<div class="update-profile">
   <?php
     $user_id=isset($_GET['user_side_profile'])?$_GET['user_side_profile']:'';
      $select = mysqli_query($connect, "SELECT * FROM user WHERE user_id = '$user_id' and role_id=2") or die('query failed');
      if(mysqli_num_rows($select) > 0){
         $fetch = mysqli_fetch_assoc($select);
      }
   ?>
   <form action="" method="post" enctype="multipart/form-data">
    <a href="logout.php" class="delete-btn w-25" style="margin-left: 70%;border-radius: 5%;text-decoration: none">Log Out</a>
        <h3 style="color:green;"> <?php echo isset($message)?($message):""; ?> </h3>
        <h3 style="color:red;"> <?php echo isset($msg)?($msg):""; ?> </h3>
      <?php
         if(isset($fetch['role_id'])==2){
      ?>
